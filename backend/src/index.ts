...
const normalizeSkills = (skillsString: string): string => {
...
app.post('/api/projects', async (req, res) => {
  try {
    ...
    const result = await db.run(
      'INSERT INTO projects (name, company, role, startDate, endDate, description, rating) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [name, company, role, startDate, endDate, description, rating]
    );
...
app.get('/api/projects/insights/skills', async (req, res) => {
  try {
    ...
    for (const proj of projects) {
      if (!proj.role || !proj.startDate) continue;
      ...
      if (durationMonths <= 0) durationMonths = 1; // minimum 1 month credit

      const roles = proj.role.split(',').map((r: string) => r.trim());
...
app.put('/api/projects/:id', async (req, res) => {
  try {
    const { name, company, role, startDate, endDate, description, rating } = req.body;
    await db.run(
      'UPDATE projects SET name = ?, company = ?, role = ?, startDate = ?, endDate = ?, description = ?, rating = ? WHERE id = ?',
      [name, company, role, startDate, endDate, description, rating, req.params.id]
    );
...
app.post('/api/contacts', async (req, res) => {
  ...
  try {
    const result = await db.run(
      ...
      [firstName, lastName, email, phone, cleanSkills, notes, relationshipStrength, lastContacted]
    );
    res.status(201).json({ id: result.lastID });
  } catch (error) {
  ...
  try {
    ...
    for (const group of duplicateGroups) {
      const contacts = await db.all(
        'SELECT * FROM contacts WHERE LOWER(firstName) = ? AND LOWER(lastName) = ? ORDER BY id ASC',
        [group.fn, group.ln]
      );
...
app.post('/api/contacts/merge', async (req, res) => {
  try {
    ...
    for (const dup of duplicates) {
      if (dup.skills) allSkills.push(dup.skills);
      if (dup.notes) allNotes.push(`[Merged Notes]: ${dup.notes}`);
    }
    ...
    await db.run(
      'UPDATE contacts SET skills = ?, notes = ? WHERE id = ?',
      [mergedSkills, mergedNotes, primaryContactId]
    );
      ...
      for (const proj of projects) {
        try {
          await db.run(
            ...
            [proj.projectId, primaryContactId, proj.role]
          );
        } catch (e) {
...
app.put('/api/contacts/:id', async (req, res) => {
  ...
  try {
    await db.run(
      'UPDATE contacts SET firstName = ?, lastName = ?, email = ?, phone = ?, skills = ?, notes = ?, relationshipStrength = ?, lastContacted = ? WHERE id = ?',
      [firstName, lastName, email, phone, cleanSkills, notes, relationshipStrength, lastContacted, req.params.id]
    );
    res.json({ success: true });
  } catch (error) {
